...
metrics_mod = import_module("06_metrics")
...
def run_image(model, src):
    ...
    metrics.save_report(os.path.join(OUT_DIR, "image_report.txt"))
    return save
...
def run_video(model, src, show=True):
    ...
    metrics.print_report()
    metrics.save_report(os.path.join(OUT_DIR, "video_report.txt"))
    return save
...
def run_webcam(model, cam_index=0):
    ...
    cv2.destroyAllWindows()
    metrics.print_report()
    metrics.save_report(os.path.join(OUT_DIR, "webcam_report.txt"))
